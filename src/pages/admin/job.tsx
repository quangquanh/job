...
const JobPage = () => {
  ...
  const companyId = JSON.parse(localStorage.getItem("user") as string)?.company
    ?._id;
  let jobs = useAppSelector((state) => state.job.result);
  jobs = companyId ? jobs.filter((v) => v?.company?._id === companyId) : jobs;
